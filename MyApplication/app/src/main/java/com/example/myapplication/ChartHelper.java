...
public class ChartHelper implements OnChartValueSelectedListener {
    ...
    public void addEntry(float value, float value2) {
        ...
        if (data != null){
            ...
            if (set == null) {
                set = createSet("alcool",0,255,0);
                data.addDataSet(set);
            }

            if (set1 == null) {
                set1 = createSet("gaz",255,0,0);
                data.addDataSet(set1);
            }

            data.addEntry(new Entry(set.getEntryCount(),value),0);
            Log.w("chart", set.getEntryForIndex(set.getEntryCount()-1).toString());
            data.addEntry(new Entry(set1.getEntryCount(),value2),1);
            Log.w("chart", set.getEntryForIndex(set.getEntryCount()-1).toString());
    ...
    private LineDataSet createSet(String label, int rouge, int green, int bleu) {
